...
> Answer: `--rm` is the tag that has the text "Automatically remove the container when it exits".
...
> Answer: The version of the package *wheel* is 0.45.1.
...
> Answer: Not Sure: I'm getting zero records for this date.
...
df[df['date'] == pd.Timestamp('2019-09-18')].shape[0]
```
...
> Answer: 2019-09-27(No sure why I'm getting this date)

> Solution (Python):

1. Calculate the trip distance for each day.

```python
df.groupby('date')['trip_distance'].max()
```

2. Get the date with the maximum trip distance.

```python
df.groupby('date')['trip_distance'].max().idxmax()
```
